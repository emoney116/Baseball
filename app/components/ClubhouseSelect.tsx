...
function useOverlayPosition(
  ...
  const updatePosition = useCallback(() => {
    ...
    if (shouldUseSheet) {
      const maxHeight = Math.min(560, Math.max(0, viewportHeight - VIEWPORT_PADDING * 2));
      setPosition({
        dialog,
        top: viewportBottom - VIEWPORT_PADDING,
        left: viewportLeft + 8,
...
function ClubhouseOverlay({
  ...
  const contextual = className.includes("clubhouse-option-overlay--context");

  useEffect(() => {
    const overlay = overlayRef.current;
    if (!dialog || !overlay || !overlay.showPopover) return;
    overlay.showPopover();
    return () => { if (overlay.isConnected && overlay.matches(":popover-open")) overlay.hidePopover(); };
  }, [dialog]);

  useEffect(() => {
    if (!contextual) return;
    const frame = window.requestAnimationFrame(() => overlayRef.current?.querySelector<HTMLElement>('input:not(:disabled), button:not(:disabled), [tabindex="0"]')?.focus());
  ...
  useEffect(() => {
    const dismissOnOutsidePress = (event: PointerEvent) => {
      ...
      if (target && (overlayRef.current?.contains(target) || triggerRef.current?.contains(target))) return;
      onDismiss();
    };
    ...
    const dismissOnEscape = (event: KeyboardEvent) => {
      ...
      // Close only the picker, leaving its parent sheet or dialog open.
  ...
  return createPortal(
    <>
      {position.placement === "sheet" && (
        <button className="clubhouse-option-overlay__scrim" type="button" aria-label={`Close ${ariaLabel}`} onClick={() => onDismiss(true)} />
      )}
      <section
        ref={overlayRef}
        popover={dialog ? "manual" : undefined}
      ...
      >
        {position.placement === "sheet" && title && (
          <header className="clubhouse-option-overlay__sheet-head">
            <strong>{title}</strong>
            <button type="button" onClick={() => onDismiss(true)} aria-label={`Close ${title}`}><X size={17} aria-hidden="true" /></button>
          </header>
        )}
        {children}
      </section>
...
export function ClubhouseSelect({
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      else (listRef.current?.querySelector<HTMLElement>('[aria-selected="true"]:not(:disabled)') ?? listRef.current?.querySelector<HTMLElement>('[role="option"]:not(:disabled)'))?.focus();
    });
    return () => window.cancelAnimationFrame(frame);
  }, [open, positioned, searchable]);

  function focusOption(index: number) {
    const optionsInList = Array.from(listRef.current?.querySelectorAll<HTMLButtonElement>('[role="option"]:not(:disabled)') ?? []);
  ...
  return (
    <div ref={rootRef} className={["choice-select", "clubhouse-select", open ? "open" : "", className].filter(Boolean).join(" ")} data-label={ariaLabel ?? label} data-empty={!selected ? "true" : undefined}>
      ...
      {open && position && !disabled && (
        <ClubhouseOverlay triggerRef={buttonRef} position={position} title={sheetTitle ?? label ?? ariaLabel} ariaLabel={ariaLabel ?? label ?? "Options"} onDismiss={close} className="clubhouse-option-overlay--select">
          {searchable && (
            <label className="clubhouse-option-overlay__search">
              <Search size={15} aria-hidden="true" />
              <input ref={searchRef} value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "ArrowDown") { event.preventDefault(); focusOption(0); } }} placeholder={searchPlaceholder} aria-label={searchPlaceholder} />
            </label>
          )}
          <div ref={listRef} id={listboxId} className="clubhouse-option-overlay__list" role="listbox" tabIndex={-1} aria-label={ariaLabel ?? label} onKeyDown={handleListKeyDown}>
            {visibleOptions.length ? visibleOptions.map((option) => (
              <button
                ...
                onClick={() => {
                  if (!buttonRef.current?.matches(":disabled")) onChange(option.value);
                  close(true);
                }}
              >
  ...
  return (
    <div className={["choice-select", "clubhouse-select", "clubhouse-multi-select", open ? "open" : "", className].filter(Boolean).join(" ")}>
      ...
      <button
        ...
        aria-expanded={open}
        aria-label={ariaLabel ?? label}
        onClick={() => { if (open) close(true); else { setDraft(values); updatePosition(); setOpen(true); } }}
      >
        <strong>{summary}</strong><ChevronDown size={16} aria-hidden="true" />
      </button>
      {open && position && !disabled && (
        <ClubhouseOverlay triggerRef={triggerRef} position={position} title={label ?? ariaLabel ?? "Options"} ariaLabel={ariaLabel ?? label ?? "Options"} onDismiss={close} className="clubhouse-option-overlay--multi">
          ...
          <div ref={listRef} className="clubhouse-option-overlay__list" role="listbox" tabIndex={-1} aria-multiselectable="true" aria-label={ariaLabel ?? label} onKeyDown={(event) => {
            ...
            const next = event.key === "Home" ? 0 : event.key === "End" ? buttons.length - 1 : event.key === "ArrowDown" ? index + 1 : event.key === "ArrowUp" ? index - 1 : null;
          ...
          <footer className="clubhouse-option-overlay__actions">
            <button type="button" className="secondary-button" onClick={() => close(true)}>Cancel</button>
            <button type="button" className="primary-button" onClick={() => { if (!triggerRef.current?.matches(":disabled")) onApply(draft); close(true); }}>Apply{draft.length ? ` (${draft.length})` : ""}</button>
          </footer>
        </ClubhouseOverlay>
      )}
    </div>
...
export function ClubhouseOptionSheet({
  ...
  const close = useCallback((returnFocus = false) => {
    onOpenChange(false);
    if (returnFocus) window.setTimeout(() => triggerRef.current?.querySelector<HTMLElement>('button, [tabindex="0"]')?.focus(), 0);
  }, [onOpenChange]);
  ...
  return <div ref={triggerRef} className="clubhouse-option-sheet-anchor">
    {trigger}
    {open && position && <ClubhouseOverlay triggerRef={triggerRef} position={position} title={title} ariaLabel={title} className={`clubhouse-option-overlay--context ${className}`} onDismiss={close}>{children}</ClubhouseOverlay>}
  </div>;
}
