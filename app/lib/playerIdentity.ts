  ...
  return {
    ...
    data: {
      ...
      hittingEvents: data.hittingEvents.map((event) => ({
        ...
        liveBpContext:event.liveBpContext?{...event.liveBpContext,before:{...event.liveBpContext.before,
          runnerIds:event.liveBpContext.before.runnerIds?Object.fromEntries(Object.entries(event.liveBpContext.before.runnerIds).map(([base,id])=>[base,canonicalId(id)??id])):undefined}}:undefined,
      })),
      practiceRunnerActions:data.practiceRunnerActions?.map(a=>({...a,runnerId:canonicalId(a.runnerId)})),
