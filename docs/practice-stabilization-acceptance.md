...
**Deployment prerequisite:** migration `20260920230057_practice_runner_analytics_projection.sql` must be applied through the normal authorized deployment process before this branch is used against a hosted database. It was tested in local PGlite only. The branch intentionally does not silently fall back to incomplete runner totals when the RPC is unavailable. Do not promote application code without its read projection.
