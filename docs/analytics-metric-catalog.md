...
| Rule | Definition |
| --- | --- |
| Game box score + tracked field data | Game-only metrics (for example PA and P/PA) coexist with Practice/Live BP tracking metrics (for example SW and Contact%) in a mixed table. They are never given a shared denominator. |
...
| Pitches per Plate Appearance | P/PA | Pitches per Plate Appearance | `pitches_per_plate_appearance` | Hitting | Confirmed pitches in completed linked game pitch sequences / those completed PAs | Yes | No | No | Supported | Requires a terminal linked pitch sequence; otherwise `—` |
...
| Stolen Base Attempts | SBA | Stolen Base Attempts | `stolen_base_attempts` | Hitting | SB + CS | Yes | No | No | Supported | None |
...
| Batted-ball mix | GB% / LD% / FB% / PU% / GB/FB | Batted-ball distribution | `ground_ball_rate`, `line_drive_rate`, `fly_ball_rate`, `pop_up_rate`, `ground_ball_to_fly_ball` | Hitting | Classified type count / classified BIP; GB/FB | Yes when contact type exists | Yes | Yes | Supported / partial game | Missing scored contact type produces `—`, never 0% |
...
| Contact allowed | BIP / GB / LD / FB / PU / GB% / LD% / FB% / PU% / GB/FB / Air% | Pitcher Contact Allowed | `balls_in_play_allowed`, `ground_ball_rate_allowed`, `line_drive_rate_allowed`, `fly_ball_rate_allowed`, `pop_up_rate_allowed`, `ground_ball_to_fly_ball_allowed`, `air_ball_rate_allowed` | Pitching | Direct scored or tagged BIP; type rate denominator is classified BIP | Yes when Game contact type exists | Yes when PitchEvent batted-ball type exists | Yes when PitchEvent batted-ball type exists | Supported / partial | Missing type produces `—`, never 0% |
| Contact quality allowed | Hard / Hard% / Soft% | Pitcher Contact Quality Allowed | `hard_contact_allowed`, `hard_contact_rate_allowed`, `soft_contact_rate_allowed` | Pitching | Quality-tagged BIP / quality-classified BIP | No | Yes | Yes | Supported / partial | Game has no contact-quality tag |
| Exit velocity and spray allowed | Avg EV / Med EV / EV90 / Max EV / Pull% / Mid% / Oppo% | Pitcher EV and Direction Allowed | `average_exit_velocity_allowed`, `median_exit_velocity_allowed`, `exit_velocity_90th_percentile_allowed`, `maximum_exit_velocity_allowed`, `pull_rate_allowed`, `middle_rate_allowed`, `opposite_rate_allowed` | Pitching | Linked hitter EV and direction records | No | No | No | Requires primitive | PitchEvent has no exit velocity/direction and safe cross-session linkage is not yet complete |
