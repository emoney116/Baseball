...
test("dense stat tables use the shared player identity and compact Weight Room headers", () => {
  ...
  assert.match(analyticsTable, /minmax\(144px, 1\.5fr\)/);
  assert.match(analyticsTable, /minTableWidth = 414/);
  assert.match(analyticsTable, /<strong>Team<\/strong>/);
  assert.doesNotMatch(analyticsTable, /analytics-team-mark|Weighted totals/);
  assert.match(page, /cell\?\.display\?\.endsWith\("%"\) \? cell\.display\.slice\(0, -1\)/);
  assert.match(page, /Contact%": "CT%"/);
  assert.match(page, /label\.slice\(0, 4\)/);
  assert.match(page, /titleAdornment=\{<AskClubhouseLauncher compact/);
  ...
  assert.match(page, /<AnalyticsInsights result=\{result\}/);
  assert.doesNotMatch(page, /<AnalyticsSummaryStrip result=\{result\}/);
  assert.match(page, /\{ value: "season", label: "All" \}/);
  assert.match(page, /\{ value: "7d", label: "Last 7" \}/);
  assert.match(page, /\{ value: "30d", label: "Last 30" \}/);
  assert.doesNotMatch(page, /aria-label="Analytics time range"/);
  assert.match(page, /function analyticsEventMatchesTimeRange/);
