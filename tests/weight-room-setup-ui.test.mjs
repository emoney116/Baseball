...
test("active weight room setup keeps exercise saves and preset UI clean", () => {
  ...
  assert.match(page, /<WeightRoomLeaderCard data=\{data\} onPlayer=\{onOpenPlayer\}/);
...
test("shared dropdown menus stay viewport safe inside modals and small screens", () => {
  ...
  assert.match(choiceSelect, /ClubhouseSelect as ChoiceSelect/);
  const shared = readFileSync("app/components/ClubhouseSelect.tsx", "utf8");
  const sharedCss = readFileSync("app/select-system.css", "utf8");
  assert.match(shared, /createPortal\(/);
  assert.match(shared, /getBoundingClientRect\(\)/);
  assert.match(shared, /visualViewport/);
  assert.match(shared, /availableBelow/);
  assert.match(shared, /availableAbove/);
  assert.match(shared, /window\.addEventListener\("scroll", updatePosition, true\)/);
  assert.match(shared, /window\.addEventListener\("orientationchange", updatePosition\)/);
  assert.match(shared, /triggerRef\.current\?\.contains\(target\)/);
  assert.match(shared, /overlayRef\.current\?\.contains\(target\)/);
  assert.match(shared, /\[open, positioned, searchable\]/);
  assert.match(sharedCss, /transform: translateY\(-100%\)/);
  assert.match(sharedCss, /overscroll-behavior: contain/);
  assert.doesNotMatch(sharedCss, /max-height:.*!important/);
  ...
  assert.equal(orgChoiceSelect, "");
  assert.match(orgManage, /import \{ ChoiceSelect \} from/);
