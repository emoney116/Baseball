...
test("shared dropdown menus stay viewport safe inside modals and small screens", () => {
  ...
  assert.match(choiceSelect, /didInitialSelectedScrollRef/);
  assert.match(choiceSelect, /if \(!open\) didInitialSelectedScrollRef\.current = false/);
  assert.match(choiceSelect, /if \(!open \|\| !menuPosition \|\| didInitialSelectedScrollRef\.current\) return/);
  assert.match(choiceSelect, /didInitialSelectedScrollRef\.current = true/);
  assert.doesNotMatch(choiceSelect, /const handleMenuWheel/);
  assert.doesNotMatch(choiceSelect, /onWheel=\{handleMenuWheel\}/);
  assert.doesNotMatch(choiceSelect, /menu\.scrollTop/);
  assert.doesNotMatch(choiceSelect, /event\.stopPropagation\(\)/);
  assert.match(choiceSelect, /event\.key === "Enter" \|\| event\.key === " "/);
  assert.match(choiceSelect, /scrollIntoView\(\{\s*block:\s*"nearest"\s*\}\)/);
  assert.equal([...choiceSelect.matchAll(/scrollIntoView\(\{\s*block:\s*"nearest"\s*\}\)/g)].length, 2);
  ...
  assert.match(orgChoiceSelect, /didInitialSelectedScrollRef/);
  assert.doesNotMatch(orgChoiceSelect, /const handleMenuWheel/);
  assert.doesNotMatch(orgChoiceSelect, /onWheel=\{handleMenuWheel\}/);
  assert.doesNotMatch(orgChoiceSelect, /menu\.scrollTop/);
  assert.equal([...orgChoiceSelect.matchAll(/scrollIntoView\(\{\s*block:\s*"nearest"\s*\}\)/g)].length, 2);
