...
test("practice hub opens active tracker modes without setup screen", () => {
  ...
  assert.equal((pitchBucketBlock.match(/id: "zone_/g) ?? []).length, 9);
  assert.equal((pitchBucketBlock.match(/id: "outside_/g) ?? []).length, 12);
  assert.equal((pitchBucketBlock.match(/isZone: true/g) ?? []).length, 9);
  assert.equal((pitchBucketBlock.match(/isZone: false/g) ?? []).length, 12);
  ...
  assert.match(page, /PracticePitchLocationGrid points=\{pitchEvents\.map/);
