-- Preserve one set per active workout, not one set per exercise across the whole day.
...
begin
  ...
  if domain='workout' then
    ...
    if not found or w.status<>'ACTIVE' or w.ended_at is not null or w.started_at is null
      or w.started_at>now() or w.created_by is null then
    ...
    select * into member from public.weight_room_workout_group_members where workout_id=w.id
      and player_id=m.player_id for share;
    if found and member.participant_status in ('NOT_PARTICIPATING','SKIPPED') then
      raise exception 'Workout assignment excludes participation.' using errcode='42501'; end if;
    select st.* into station from public.weight_room_workout_stations st
      where st.workout_id=w.id and st.id=(payload->>'stationId')::uuid
        and st.archived_at is null for share of st;
    if not found or station.exercise_id is null then raise exception 'Programmed exercise required.' using errcode='42501'; end if;
  ...
  if operation='delete' then
    ...
    if operation='create' then
      ...
      if exists(select 1 from public.workout_sets where player_id=m.player_id and active_workout_id=w.id
