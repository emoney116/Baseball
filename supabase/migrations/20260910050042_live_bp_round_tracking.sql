-- Live BP coordinates existing Practice event tables; it never writes Games.
create table public.live_bp_rounds (
  id uuid primary key,
...
create index live_bp_rounds_practice on public.live_bp_rounds(practice_id,created_at);
alter table public.live_bp_rounds enable row level security;
revoke all on public.live_bp_rounds from public,anon,authenticated;
grant all on public.live_bp_rounds to service_role;

alter table public.hitting_events add column live_bp_round_id uuid references public.live_bp_rounds(id), add column live_bp_context jsonb;
alter table public.pitch_events add column live_bp_round_id uuid references public.live_bp_rounds(id), add column live_bp_context jsonb;
alter table public.defense_events add column live_bp_round_id uuid references public.live_bp_rounds(id), add column live_bp_context jsonb;
create index hitting_live_bp_round on public.hitting_events(live_bp_round_id) where live_bp_round_id is not null;
create index pitching_live_bp_round on public.pitch_events(live_bp_round_id) where live_bp_round_id is not null;
create index defense_live_bp_round on public.defense_events(live_bp_round_id) where live_bp_round_id is not null;

-- Linked events must only be mutated as a unit through the server endpoint.
create function clubhouse_private.protect_bp_link() returns trigger language plpgsql set search_path='' as $$
begin
  if current_user in ('authenticated','anon') and
    ((tg_op<>'INSERT' and old.live_bp_round_id is not null) or (tg_op<>'DELETE' and new.live_bp_round_id is not null)) then
    raise exception 'Use the Live BP endpoint.' using errcode='42501';
  end if;
  if tg_op='DELETE' then return old; end if;
  return new;
end; $$;
create trigger protect_bp_link before insert or update or delete on public.hitting_events for each row execute function clubhouse_private.protect_bp_link();
create trigger protect_bp_link before insert or update or delete on public.pitch_events for each row execute function clubhouse_private.protect_bp_link();
create trigger protect_bp_link before insert or update or delete on public.defense_events for each row execute function clubhouse_private.protect_bp_link();

create function clubhouse_private.bp_session(round_id uuid, practice uuid, actor uuid, player uuid, domain text, thrower text)
returns uuid language plpgsql security invoker set search_path='' as $$
declare found_id uuid;
begin
  select id into found_id from public.practice_sessions where practice_id=practice and player_id=player
    and category::text=domain and metadata->>'liveBpRoundId'=round_id::text and metadata->>'liveBpThrowerSource'=thrower limit 1;
  if found_id is null then
    insert into public.practice_sessions(practice_id,player_id,category,session_type,created_by_profile_id,entry_policy,metadata)
      values(practice,player,domain::public.practice_session_category,'Live BP',actor,'COACH_ONLY',
        jsonb_build_object('liveBpRoundId',round_id,'liveBpThrowerSource',thrower)) returning id into found_id;
  end if;
  return found_id;
end; $$;
revoke all on function clubhouse_private.bp_session(uuid,uuid,uuid,uuid,text,text) from public,anon,authenticated;
grant execute on function clubhouse_private.bp_session(uuid,uuid,uuid,uuid,text,text) to service_role;

create function public.write_live_bp(actor uuid, practice uuid, round_id uuid, operation text, expected_version integer, request_id uuid, payload jsonb)
...
begin
  ...
  if operation='start' then return r;
    ...
    insert into public.hitting_events(id,practice_id,session_id,hitter_id,pitcher_id,event_number,action,contact_result,field_location,pitch_location,pitch_type,velocity,exit_velocity_mph,is_live_bp,context,created_by_profile_id,idempotency_key,live_bp_round_id,live_bp_context)
      values(request_id,p.id,session,(s->>'hitterId')::uuid,case when source='PLAYER' then (s->>'pitcherId')::uuid end,r.version+1,h.action,h.contact_result,h.field_location,h.pitch_location,h.pitch_type,h.velocity,h.exit_velocity_mph,true,'live_bp',actor,request_id::text,r.id,payload->'context');
